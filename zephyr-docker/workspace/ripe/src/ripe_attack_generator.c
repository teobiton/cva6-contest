...
void shellcode_target();
void ret2libc_target();
void rop_target();
void dop_target(char *buf, uint32_t auth);

// JM: contains buffer lower in memory than stack param, allowing for overflow
void set_low_buf(char **buf);

// JM: integer overflow vulnerability
void iof(char *buf, uint32_t iv);

// JM: arbitrary read bug
void data_leak(char *buf);

// JM: forces length param to register and jumps before return for stack param attacks
void homebrew_memcpy_param(void *dst, const void *src, register size_t length);

// JM: longjmp() is called from here
void lj_func(jmp_buf lj_buf);
...
void print_current_test_parameters(void)
{
...
#define PMP_LOCK (1 << 7)
void my_test_pmp_write(void)
{
    uint64_t add_buffer;
    printf("Start my test pmp\n");
    ...
    add_buffer = (uint64_t)&vul_buffer[16]; /* addr is 34 bits on RV32 */
    add_buffer = (add_buffer >> 2) & 0xffffffff;
    write_csr(pmpaddr0, (uint32_t)add_buffer);
    ...
    printf("Failed to catch writing in restricted area \n");
}

void my_exec_attack (void){
    printf("Failed to catch execution in restricted area \n");
}

void my_test_pmp_exec(void){
    uint64_t add_buffer;

        /* Set address in the middle a buffer */
    add_buffer = (uint64_t)&my_exec_attack; /* addr is 34 bits on RV32 */
    add_buffer = (add_buffer >> 1) & 0xffffffff;
    write_csr(pmpaddr0, (uint32_t)add_buffer);
     ...
     * Naturally aligned 4 bytes region
     * Active read, disable write and exec */
    write_csr(pmpcfg0, (PMP_LOCK | PMP_NAPOT | PMP_R | PMP_W));
    ...
    */
    ...
    case DIRECT:
        ...
        default:
            if (output_debug_info)
            {
                printf("Unknown choice of attack code");
                exit(1);
            }
        }
        break;
    ...
    {
        payload.size =
            (unsigned int)((unsigned long)target_addr + sizeof(long) - (unsigned long)buffer + 1); /* For null termination so that buffer can be     */
    ...
    {
        if (output_debug_info)
            printf("Error calculating size of payload\n");
        exit(1);
    }
    ...
    case INDIRECT:
        ...
        if (attack.inject_param == RETURN_INTO_LIBC)
        {
            ...
            payload.overflow_ptr = &ret2libc_target;
            payload.size = (uintptr_t)target_addr_aux - (uintptr_t)buffer + sizeof(long) + 1;
...
=======
    char *shellcode, *temp_char_buffer, *temp_char_ptr;
>>>>>>> f781c76 (RIPE project modifications)
...
is_attack_possible()
{
    ...
    if (attack.inject_param == DATA_ONLY)
    {
        if (attack.code_ptr != VAR_BOF &&
            ...
            attack.code_ptr != VAR_LEAK)
        {
            printf("Error: Misused DOP code pointer parameters.\n");
            return FALSE;
        }
